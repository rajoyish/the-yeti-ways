...
- Picture at the handoff. The first beat of a video's Shot 1 holds the picture the previous video's last beat ended on: the same positions, paws, props, eye lines, and faces, seen through the new shot's opening framing, with at most a small change (a face that starts to turn, a breath, a glance). Panel image 1 of each video after the first shows the same picture as panel image 6 of the video before it. A video marked `Scene break:` opens on its new location instead.
...
- Effects in a panel image are frozen: flakes hang in the air, embers are points of light, spray is drops in mid-air. Slow motion, camera moves, and transitions do not exist in a still image.
...
| 1. Storyboard | `storyboard-30s-extender` for 30 seconds, `storyboard-60s-from-story` for 60 | A short storyboard, a story, or a source storyboard | `prompts/<slug>-<30s or 60s>-storyboard.md`: one `Character locks:` block and one table per video, three shots, two beats per shot. The block and the table together are that video's Flow prompt, and the file holds no image prompt |
| 2. Images | `create-3x2-timed-image` | The storyboard | `prompts/<slug>-<30s or 60s>-3x2.md`, every image prompt for the film: per video, six panel image prompts, one per panel. The panel images are generated first, then each video's grid is composed from them by `compose_grid.ps1`. The images go in `3x2-timed-storyboard-images/<slug>/`, as `Panel_<P>_V<N>.jpg` for each panel image and `Grid_V<N>.jpg` for each grid, where `V` is the video number |
...
Step 3 has two routes, and the operator picks one per video. Both paste the video's `Character locks:` block and table as the text prompt, and both attach images as references, never as a start or an end frame. Flow accepts at most seven images for one clip:
...
- Grid route: attach the video's composed 3x2 grid and the family reference image. The grid shows Flow all six beats in order in one picture, and because it is composed from the panel images, it shows exactly the pictures the panel route would.

The storyboard is the single source. The panel images are drawn from its beats, lights, framings, and effects, never from a picture that exists only in the head of whoever writes them, and the grids are composed from the panel images. When a beat changes, change it in the storyboard first, then rewrite and regenerate that video's panel images and compose its grid again. The storyboard holds the video prompts and the 3x2 file holds the image prompts, and neither holds the other's. Run each step as its own skill run, in order. One step's saved file is the next step's input, and a single run that writes the storyboard and the image prompts together runs past a model's output limit.
